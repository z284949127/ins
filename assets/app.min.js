...
`;let InsCrack={name:"InsCrack",data(){return{list:[],isEdit:false,schema:"破解配置",model:{crackName:"ins破解",crackCode:`
        async function doCrack(__this,axios) {
            ...
            内置函数说明:
                getForText(url)     封装获取url文本内容
                logMsg(msg)         打印日志,页面上显示破解实时日志
                getForHomeHtml()    封装获取ins首页html文本
                parser              一个DomParser的实例,用于解析html为dom对象,解析html中的引用js文件链接,即 parser = new DomParser()
                crackMsg            破解过程中页面上的日志信息数组,破解成功后清空一下crackMsg.clear()或者__this.crackMsg=[]
                __this.queryHash    保存破解后的queryHash(不用处理,这里封装了一个saveHash()方法)
                sharedState         Vue3 简单状态管理, 保存破解后的queryHash(不用处理,这里封装了一个saveHash()方法)
            */
            let {getForText,logMsg,getForHomeHtml,parser,sharedState,crackMsg} = __this
            let saveHash=(queryHash)=>{
                __this.queryHash = queryHash
                __this.sharedState.queryHash = __this.queryHash
            }
            logMsg('开始破解[queryHash]')
            let srcArr = [];
            await getForHomeHtml().then(html => {
                if (html) {
                    logMsg('已获取首页数据')
                } else {
                    logMsg('获取首页数据失败')
                ...
                srcArr = Array.from(scriptElements).filter(el => el.src).map(el => el.src).filter(sr => sr.indexOf('rsrc.php') > 0);
                if (srcArr.length > 0) {
                    // 已获取密码地址
                    logMsg('已获取到破解源')
                } else {
            ...
            setTimeout(() => __this.crackMsg = [], 10000)
        }
        /* 导出破解方法,这行不可删改 */
        that.that = doCrack;`,enable:true},listShowModel:{crackName:{type:"text",maxLength:15},crackCode:{type:"text",maxLength:50},enable:{type:"boolean",default:true}},formShowModel:{crackName:{type:"text",maxLength:15},crackCode:{type:"textarea",maxLength:50},enable:{type:"boolean",default:true}},editModel:{}}},components:{List:List,Form:Form},template:template,mounted:()=>{console.log("ins-crack mounted");const{proxy}=Vue.getCurrentInstance();proxy.doGetAll()},methods:{doRefresh(){this.$api.insCrack.getAll().then(res=>{if(res.data&&res.data.code===0){this.list=res.data.data;let msg="刷新成功";this.$message({message:msg})}else{let msg="接口调用失败";console.error(msg,res);this.$message({message:msg,level:"error"})}}).catch(e=>{let msg="接口调用失败!";console.error(msg,e);this.$message({message:msg,level:"error"})})},doGetAll(){this.$api.insCrack.getAll().then(res=>{if(res.data&&res.data.code===0){this.list=res.data.data}else{let msg="接口调用失败";console.error(msg,res);this.$message({message:msg,level:"error"})}}).catch(e=>{let msg="接口调用失败!";console.error(msg,e);this.$message({message:msg,level:"error"})})},doAdd(){this.editModel=deepClone(this.model);this.isEdit=true},doEdit(item){this.editModel=deepClone(item);this.isEdit=true},doDelete(item){this.$api.insCrack.delete(item).then(res=>{if(res.data&&res.data.code===0){let msg="删除成功!";console.log(msg,res.data);this.$message({message:msg});this.isEdit=false;this.doGetAll()}else{let msg="删除失败!";console.error(msg,res.data);this.$message({message:msg,level:"error"})}}).catch(e=>{let msg="删除失败!";console.error(msg,e);this.$message({message:msg,level:"error"})})},doSubmit(){this.$api.insCrack.update(this.editModel).then(res=>{if(res.data&&res.data.code===0){let msg="保存成功!";console.log(msg,res.data);this.$message({message:msg});this.isEdit=false;this.doGetAll()}else{let msg="保存失败!";console.error(msg,res.data);this.$message({message:msg,level:"error"})}}).catch(e=>{let msg="保存失败!";console.error(msg,e);this.$message({message:msg,level:"error"})})},doCancel(){this.isEdit=false},doStart(){alert("start")}}};var template=`
<div class="config">
<h3>设置管理</h3>
<button @click="doAdd" v-if="!isEdit && list.length === 0">新增</button>
<button @click="doCancel" v-if="isEdit">取消</button>
...
<router-link to="/crack">破解配置</router-link>
