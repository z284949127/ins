...
`;let Message={name:"Message",data(){return{list:[]}},props:{message:{type:[String,Array],required:true},level:{type:String,default:"success"},duration:{type:Number,default:3}},setup(props){let{ref}=Vue;let isShow=ref(false);const show=()=>{isShow.value=true};const close=()=>{isShow.value=false};return{isShow:isShow,show:show,close:close}},created(){this.show()},template:template,watch:{isShow(nVal,oVal){}}};var template=`
<div class="task">
<h3>任务管理</h3>
...
`}},components:{},template:template,mounted:()=>{const{proxy}=Vue.getCurrentInstance();console.log("%s mounted",proxy.$options.name);proxy.doGetCss()},methods:{doGetCss(){this.$api.style.getApp().then(res=>{if(res.status===200&&res.data){this.cssData=res.data}else{let msg="接口调用失败";console.error(msg,res);this.$message({message:msg,level:"error"})}}).catch(e=>{let msg="接口调用失败";console.error(msg,e);this.$message({message:msg,level:"error"})})}}};var template=`
<div>
<div class="nav">
